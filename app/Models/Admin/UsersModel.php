...
class UsersModel extends Model
{
    ...
    public function updateUser($post)
    {
        ...
        $update = [
            'name' => trim($post['name']),
            'email' => trim($post['email']),
            'billing_address' => ((isset($post['billing_address']) && !empty($post['billing_address'])) ? $post['billing_address'] : ''),
            'delivery_address' => ((isset($post['delivery_address']) && !empty($post['delivery_address'])) ? $post['delivery_address'] : '')
        ];
        if ($password !== false) {
