...
// change the size of product
Route::post('changeSize', 'Publics\\CartController@changeSize');
...
Route::get('admin/login', [
    'as' => 'admin.login',
    'uses' => 'Auth\AdminLoginController@showAdminLoginForm'
]);
Route::post('admin/login', [
    'as' => 'admin.login',
    'uses' => 'Auth\AdminLoginController@login'
]);
Route::get('admin/logout', ['uses' => 'Admin\\UsersController@logout']);
...
/* Customer Routes */
Route::middleware(['auth.customer'])->group(function () { // check for autherization
    Route::get('customer', 'Customer\\AccountController@index');
    Route::get('{locale}/customer', 'Customer\\AccountController@index')
            ->where('locale', implode('|', Config::get('app.locales')));  

    ///////////
    Route::get('customer/orders', 'Customer\\OrdersController@index')->where('locale', implode('|', Config::get('app.locales')));
    Route::get('{locale}/customer/orders', 'Customer\\OrdersController@index');

    /////////
    Route::get('customer/returns', 'Customer\\ReturnsController@index')->where('locale', implode('|', Config::get('app.locales')));
    Route::get('{locale}/customer/orders', 'Customer\\ReturnsController@index');

    /////////
    Route::get('customer/returning', 'Customer\\ReturningController@index')->where('locale', implode('|', Config::get('app.locales')));
    Route::get('{locale}/customer/returning', 'Customer\\ReturningController@index');

    /////////
    Route::get('customer/discount', 'Customer\\DiscountController@index')->where('locale', implode('|', Config::get('app.locales')));
    Route::get('{locale}/customer/discount', 'Customer\\DiscountController@index');

    /////////
    Route::get('customer/charge', 'Customer\\ChargeController@index')->where('locale', implode('|', Config::get('app.locales')));
    Route::get('{locale}/customer/charge', 'Customer\\ChargeController@index');

    /////////
    Route::post('customer/charge', 'Customer\\ChargeController@payWithStripe')->where('locale', implode('|', Config::get('app.locales')));
    Route::post('{locale}/customer/charge', 'Customer\\ChargeController@payWithStripe');

    /////////
    Route::get('customer/chargewithpaypal', 'Customer\\ChargeController@payWithPayPal')->where('locale', implode('|', Config::get('app.locales')));
    Route::get('{locale}/customer/chargewithpaypal', 'Customer\\ChargeController@payWithPayPal');

    Route::get('paypalsuccess', 'Customer\\ChargeController@paypalSuccess');
    Route::get('paypalcancel', 'Customer\\ChargeController@paypalCancel');

    /////////
    // return order from orders
    Route::post('returnOrder', 'Customer\\OrdersController@returnOrder');

    // leave a review
    Route::post('leaveReview', 'Customer\\OrdersController@leaveReview');

    // set user 
    Route::post('updateCustomer', 'Customer\\AccountController@updateCustomer');
...
Route::get('customer/login', [
    'as' => 'customer.login',
    'uses' => 'Auth\CustomerLoginController@showCustomerLoginForm'
]);
Route::post('customer/login', [
    'as' => 'customer.login',
    'uses' => 'Auth\CustomerLoginController@login'
]);
Route::get('customer/logout', ['uses' => 'Customer\\UsersController@logout']);

Route::get('customer/register', [
    'as' => 'customer.register',
    'uses' => 'Auth\CustomerRegisterController@showRegistrationForm'
]);
Route::post('customer/register', [
    'as' => 'customer.register',
    'uses' => 'Auth\CustomerRegisterController@register'
]);
